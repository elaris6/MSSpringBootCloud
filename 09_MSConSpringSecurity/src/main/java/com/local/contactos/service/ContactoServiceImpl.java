...
public class ContactoServiceImpl implements ContactoService {
	...
	public void agregarContacto(Contacto contacto) throws Exception{
		 ...
		 * mediante el método de recuperación de Email. */
		if(dao.recuperarContacto(contacto.getEmail()) == null) {
			dao.agregarContacto(contacto);
		} else {
		/* En caso de encontrar que ya existe un contacto con ese email, se lanza
		 * una excepción de tipo "Exception" (aunque que podría ser una personalizada
		 * para nuestra aplicación), con un mensaje personalizado.
		 * Este mensaje luego se recuperará mediante el método "getMEssage()" asociado
